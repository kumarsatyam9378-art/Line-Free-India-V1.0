...
export default function BottomNav() {
  ...
  // Background styles for the nav bar - theme-aware
  const navStyles = "flex justify-around items-center h-[68px] pointer-events-auto relative bg-card/80 backdrop-blur-2xl rounded-[22px] border border-border shadow-2xl transition-colors duration-300";
  ...
  if (role === 'customer') {
    ...
    return (
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] md:max-w-2xl lg:max-w-4xl z-[100] px-4 pb-6 pointer-events-none">
          ...
          {tabs.map(tab => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[8px] font-black z-10 uppercase tracking-[1px] transition-colors duration-300 ${active ? 'text-primary' : 'text-text-dim'}`}>
                  {tab.label}
                </span>
              </button>
            );
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[7px] font-black z-10 uppercase tracking-[1px] truncate w-full text-center transition-colors duration-300 ${active ? 'text-primary' : 'text-text-dim'}`}>
                    {tab.label}
                  </span>
                </button>
              );
